...
评分标准（语义承诺兑现为准；"必须包含/不得包含/格式=json"已由确定性规则轨单独校验，
你只复核规则未覆盖的语义问题——例如 JSON 键的值是否与参考材料的事实一致、是否答非所问）：
...
- 判 fail 必须指出候选回复中可逐字指认的具体缺陷（引原文片段）；
  无法核验的表述差异（例如对照一份你看不到的原文要求"逐字一致"）、以及确定性规则轨
  已覆盖的机械格式差异（如 JSON 围栏），不得作为 fail 理由。
