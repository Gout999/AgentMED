...
export DATABASE_URL="${DATABASE_URL:-sqlite:////tmp/caseloop-mcp-projection-smoke.db}"
"$PY" scripts/run_migrations.py >/tmp/caseloop-mcp-smoke-migrate.log 2>&1 || {
...
start_projection() {
  ...
  env \
    ...
    PYTHONPYCACHEPREFIX=/tmp/caseloop-mcp-smoke-pycache \
    "$port_var=$port" \
  ...
  code="$(curl -s -o /dev/null -w '%{http_code}' -X POST "http://127.0.0.1:$port/mcp" \
    -H "X-CaseLoop-Gateway-Token: $backend_token" \
...
start_projection mcp-case-admin-quality-officer case_admin quality-officer CASE_ADMIN_PORT 8101 quality-officer \
...
start_projection mcp-case-admin-case-officer case_admin case-officer CASE_ADMIN_PORT 8301 case-officer \
  '["case.get"]' no no
start_projection mcp-case-admin-attributionist case_admin attributionist CASE_ADMIN_PORT 8401 eval-runner \
  '["case.get","case.claim","app.logs"]' yes no
start_projection mcp-case-admin-repairer case_admin repairer CASE_ADMIN_PORT 8501 repairer \
  '["case.get","case.timeline","case.claim"]' yes no
start_projection mcp-release-admin-gatekeeper release_admin gatekeeper RELEASE_ADMIN_PORT 8102 gatekeeper \
  '["workorder.get","gate.submit","approval.request","approval.status","release.get"]' yes no
start_projection mcp-release-admin-repairer release_admin repairer RELEASE_ADMIN_PORT 8202 repairer \
  '["versionset.list","versionset.get","candidate.create","workorder.draft","workorder.freeze","workorder.get","release.get"]' yes no
start_projection mcp-eval-runner-gatekeeper eval_runner gatekeeper EVAL_RUNNER_PORT 8103 gatekeeper \
  '["gate.run","gate.run_verification","gate.report"]' yes yes
start_projection mcp-eval-runner-attributionist eval_runner attributionist EVAL_RUNNER_PORT 8203 eval-runner \
...
start_projection mcp-notification-quality-officer notification quality-officer NOTIFICATION_PORT 8104 quality-officer \
  '["matrix.log"]' no no
start_projection mcp-notification-case-officer notification case-officer NOTIFICATION_PORT 8204 case-officer \
