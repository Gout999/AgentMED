...
Agent 团队通过 MCP 工具操作控制面与外部世界；trust-ledger 记录「信任是挣来的」的数学证据；
...
mcp-servers/
  ...
  trust_ledger/          信任账本模块（库，非独立 server；spec §6 / §9.8）
  ...
  scripts/
    ...
    trust_demo.py        trust-ledger MVP 演示
...
- **trust-ledger**（spec §6 / §9.8）：一次动作=一个样本；epoch 原始整数计数；Wilson 双侧
  95% 下界 >0.9 且白名单 R1 才可提请晋升；R2 永远逐次审批。MVP 3/3 → 下界≈0.4385 →
  记账但拒绝晋升。SUSPENDED 冷却 24h + 人工 reinstate（D-001 Q8，不自动恢复）。
