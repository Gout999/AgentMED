...
- Q5：真飞书定位冻结为 `channel=feishu:<chat_id>`、`thread_ref=feishu:<chat_id>:<message_id>`；`message_id` 是收到投诉的原消息，也是 reply API 的目标，两个字段必须匹配且均不可为空。mock 继续使用 `feishu-mock:<room>:<msg_ref>`。见 `docs/decisions/D-001-open-questions.md`。
