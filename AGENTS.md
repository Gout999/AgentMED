...
- V5 stage construction baseline (accepted 2026-08-11): `docs/plan-v5.md` + `docs/plans/v5-progressive-delivery.md` (blueprint dependency graph) + the frozen `contracts/v5/` namespace, including the V5-0B freeze commit `8dd25ca` (system governance ownership) and the V5-0C freeze commit `b3727d7` (first system wire slice), independently accepted on 2026-08-11. `docs/plan-v4.md` remains the V4 compatibility baseline and v3 remains the implemented compatibility baseline; the V5 baseline governs V5 stage construction only and does not retroactively change V4 or v3 authority. Contract freeze is not runtime implementation: all V5 runtime facets remain `NOT_RUN` until a stage produces real evidence.
- V5 stages are built one at a time following the blueprint dependency graph. Each stage requires focused tests, a verifier pass, evidence under `evidence/v5/`, and its own semantic commit before the next stage may start. V5-1 and later stages are unfrozen on the local branch; push, PR, paid provider calls, human approvals, and production or other external writes still require their own authorization. Honest uncertainty belongs in commit bodies and `evidence/`, never hidden from a stage report.
