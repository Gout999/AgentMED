...
| G20 | `test_v5_application_catalog_postgres.py` 死锁 + CLI 安装缺 build backend | 死锁根因：断言版本时 `engine.connect()` 未关连接（:164-169），idle-in-transaction 持锁与 `DROP SCHEMA` 互等；CLI 安装根因：PEP 668/uv base python 无 setuptools，`--no-build-isolation` 找不到 build backend。两者叠加致 s8 永不完整 | ✅ **已修（2026-08-12，`f1598bd`）**：断言改 `with engine.connect() as ...`；`_install_cli` 的 install_env 加 PYTHONPATH 指向 control-plane venv site-packages（venv 内需装 setuptools）。修后 e2e ~9s 通过，`verify_convergence.sh` 首次 **8/8 全绿** |
