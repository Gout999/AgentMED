...
# contract runtime is a later slice, so V5-1C exposes an honest pending
# materialization status rather than claiming a nonexistent exact binding.
# ----------------------------------------------------------------------------
...
class IssueSnapshotRequest(WireModel):
    ...
    source_url: Annotated[str, Field(min_length=1, max_length=1024)] | None = None
    external_repo: Annotated[str, Field(min_length=1, max_length=256)] | None = None
    external_issue_number: Annotated[StrictInt, Field(ge=1)] | None = None
    ...
    @model_validator(mode="after")
    def source_identity_matches_kind(self) -> "IssueSnapshotRequest":
        if self.source_kind == "github_issue":
            if (
                self.source_url is None
                or not self.source_url.startswith(("https://", "http://"))
                or self.external_repo is None
                or self.external_issue_number is None
            ):
                raise ValueError(
                    "github issue source requires http(s) url, repo and issue number"
                )
        elif self.external_repo is not None or self.external_issue_number is not None:
            raise ValueError("manual source must not invent repository or issue identity")

        # The transport-level provenance flags are authoritative.  Copy them
        # into the data-only provider payload because the canonical normalizer
        # receives that payload downstream; a caller-supplied nested value may
        # never override the explicit envelope flags.
        canonical_payload = dict(self.snapshot_payload)
        canonical_payload["edited_flag"] = self.edited_flag
        canonical_payload["deleted_flag"] = self.deleted_flag
        self.snapshot_payload = canonical_payload
        return self
...
class AcceptanceCriteriaRevisionRecord(WireModel):
    ...
    exact_previous_proposed_revision_binding: AcceptanceCriteriaRevisionBinding | None = None
    reauthentication_credential_binding: ReauthenticationCredentialBinding | None = None
    acceptance_source: dict[str, Any]
    reproducer_input: dict[str, Any] | None = None
    reproducer_environment: dict[str, Any] | None = None
    expected_behavior: dict[str, Any]
    ...
    @model_validator(mode="after")
    def immutable_confirmation_shape(self) -> "AcceptanceCriteriaRevisionRecord":
        if (
            self.record_envelope.workspace_id != self.workspace_id
            or self.record_envelope.revision != 1
            or self.resolution_contract_binding_status.status
        ...
        confirmed_fields = (
            ...
            self.confirmed_at,
            self.exact_previous_proposed_revision_binding,
            self.reauthentication_credential_binding,
        )
        if self.confirmation_status == "PROPOSED":
            if any(value is not None for value in confirmed_fields):
                raise ValueError("proposed revision must not carry confirmation fields")
        else:
            if any(value is None for value in confirmed_fields):
                raise ValueError("confirmed revision requires exact confirmation fields")
            assert self.confirmer_principal is not None
            assert self.exact_previous_proposed_revision_binding is not None
            assert self.reauthentication_credential_binding is not None
            if (
...
class AcceptanceCriteriaConfirmRequest(WireModel):
    schema_version: SchemaVersion2
    exact_proposed_revision_binding: AcceptanceCriteriaRevisionBinding
