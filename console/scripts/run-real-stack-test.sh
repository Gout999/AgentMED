...
control_plane_venv="${CASELOOP_CONTROL_PLANE_VENV:-${control_plane_dir}/.venv}"
evidence_dir="${repo_root}/evidence/p0/p0-3-console"
audit_log="${evidence_dir}/audit.jsonl"
python_pycache_prefix="${CASELOOP_PYTHONPYCACHEPREFIX:-/tmp/caseloop-console-e2e-pycache-$$}"
...
db_name="caseloop_console_e2e_$(date +%s)_$$"
case "${db_name}" in
  caseloop_console_e2e_[0-9]*) ;;
  *)
...
pg_host="${CASELOOP_TEST_PG_HOST:-127.0.0.1}"
pg_port="${CASELOOP_TEST_PG_PORT:-5432}"
pg_user="${CASELOOP_TEST_PG_USER:-caseloop}"
pg_password="${CASELOOP_TEST_PG_PASSWORD:-caseloop}"
pg_admin_host="${CASELOOP_TEST_PG_ADMIN_HOST:-/tmp}"
pg_admin_port="${CASELOOP_TEST_PG_ADMIN_PORT:-5432}"
pg_admin_user="${CASELOOP_TEST_PG_ADMIN_USER:-$(id -un)}"
pg_admin_database="${CASELOOP_TEST_PG_ADMIN_DATABASE:-postgres}"
pg_admin_password="${CASELOOP_TEST_PG_ADMIN_PASSWORD:-}"
