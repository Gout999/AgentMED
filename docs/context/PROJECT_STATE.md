...
- `c2_convergence_status`: `ELIGIBLE / NOT STARTED` (C1 gate review 后才开始; C3–C5, D2, R3-full, R4, V5-2+ 继续 LOCKED)
...
- `last_independently_verified_semantic_subject_commit`: `3dc7339` (C1 single-source wire subject)
...
- The current execution baseline is D-015 plus `docs/plans/v5-architecture-convergence.md` and the
  ...
  convergence gate releases them. C0 closed at `903e954`; C1 closed at `3dc7339` (compiler + 2020-12 schemas,
  deterministic, shadow parity PASS); C2 is `ELIGIBLE / NOT STARTED`.
...
C0 and C1 architecture convergence waves are closed (`903e954`, `3dc7339`; verifier
PASS P0=0/P1=0); C2 is `ELIGIBLE / NOT STARTED`. R1/R2 semantic subjects are
