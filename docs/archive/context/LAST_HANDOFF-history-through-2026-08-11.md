# Last Handoff history through 2026-08-11

> 归档状态：**HISTORICAL SNAPSHOT**
>
> 原路径：`docs/context/LAST_HANDOFF.md`
>
> 本文件保留 2026-08-10 至 2026-08-11 的累计 handoff 原貌，包括归档当时的 current
> section。真正 current handoff 只看
> [`docs/context/LAST_HANDOFF.md`](../../context/LAST_HANDOFF.md)。历史测试计数、HEAD、
> Agent/runtime/live 状态不能推导为当前事实。

## 2026-08-11 V5-1A/B/C project-review repair handoff (current)

- `closure_status`: **IN_PROGRESS / NO-GO for stage DONE**. A project-wide product, contract,
  security, journey, evidence and operations review rejected the prior 1A/1B/1C closure claims.
  The old tracked evidence bundles are retained but explicitly marked `SUPERSEDED`/`REJECTED`.
  Current HEAD is `4a0a421`; all repair changes are uncommitted and no push was performed.
- `authority_and_integrity_repair`: V5 major-2 events and exact subject bindings, immutable revision
  `1`, authority receipt replay, project/environment grants, server-owned trust roles, durable
  denial audit, fresh post-proposal human credential, concurrent confirmation exclusion, manifest
  import locking, provenance/dataset/diff validation and dirty-worktree discovery now fail closed.
- `first_case_truth`: the supported local path is documented in
  `control-plane/V5_FIRST_CASE_LOCAL.md`: local bootstrap, manifest import, operator credential
  rotation, Signal/Case, exact application/version binding, acceptance proposal, independent owner
  reauthentication and confirm. Credential issuance is deliberately local management, never public
  HTTP. Confirmation remains non-executable until V5-4 materializes ResolutionContract; Console
  therefore shows NEEDS rather than false READY.
- `transport_and_contract`: `/api/v2/capabilities` now uses an audited, stage-scoped allowlist and
  the explicit-major CLI. The V5 runtime overlay preserves the historical V5-0C freeze separately
  from current transports. Standalone `system-versions.record`, V5-2+ and all MCP/A2A skeletons are
  still undiscoverable/not implemented.
- `operations_repair`: Compose requires independent public secrets, defaults published ports to
  loopback and uses DB/migration/secret-aware readiness. Demo schema ownership moved from
  `create_all` to Alembic; equal OAuth client secrets fail closed; `NOTIFICATION_LOG_URL` is now
  honored. Internal legacy read routes still need an authenticated production ingress if exposed
  beyond the local loopback stack.
- `verification`: focused repaired slices are green (control-plane application/system/capability,
  acceptance/migration/read projection; bootstrap; CLI; contracts; Console; demo/MCP/Compose as
  listed in the current review evidence). Five PostgreSQL first-case/1B/race integrations pass in a
  dedicated ephemeral loopback container and the container was removed afterward. The independent
  remediation verifier passed after capability discovery was made principal-type-aware. No live,
  provider, Agent, repo-sandbox, human-authorized external or production facet was run.
- `known_blockers`: (1) frozen AIApplication/SystemComponent state machines require
  REGISTERED→ACTIVE events, while current register/import writes ACTIVE directly; (2) standalone
  second-version `system-versions.record` is absent; (3) V5 outbox delivery belongs to V5-2;
  (4) ResolutionContract belongs to V5-4; (5) D-013 is still untracked, so clean-checkout product
  authority links remain broken.
- `next_action`: do not start V5-2. Generate a digest-bearing evidence manifest, resolve or formally
  migrate the lifecycle mismatch, deliberately include D-013, then create one semantic completion
  commit and post-commit verifier result per stage. Push/PR/live/external actions remain separately gated.
...
- `key_files`: `contracts/schemas/gate-report.schema.json`,
  ...
  `docs/decisions/D-007-synchronous-gate-report-receipt.md`,
