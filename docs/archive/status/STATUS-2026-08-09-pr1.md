...
> 原路径：`STATUS.md`
>
> 适用快照：PR #1 合并后；当前状态请读
> [`PLANS.md`](../../../PLANS.md)、
> [`PROJECT_STATE.md`](../../context/PROJECT_STATE.md) 和
> [`LAST_HANDOFF.md`](../../context/LAST_HANDOFF.md)。
>
> 本文不能证明当前 V5 runtime、live/provider、Agent 或 production 状态。
>
> 归档时仅对个人用户名、本地 secret location 和本机路径做了脱敏；没有改写历史能力结论。
...
# 2. AgentTeams 平台（agent 编排层）：装法见 deploy/README.md；历史联系人已脱敏
...
StepFun key 的历史个人存放位置已脱敏为 `[REDACTED_SECRET_LOCATION]`，
...
2. **飞书真接入**：现在是 mock + Matrix 房间代发（@caseloop-approver）。`[REDACTED_USER]` 当时在准备
...
5. **裁判模型替换**：现在门禁的 LLM 裁判也用 StepFun（运动员裁判同家，演示可接受，
   评审会问）。`[REDACTED_USER]` 当时说之后会提供别的模型 key。
