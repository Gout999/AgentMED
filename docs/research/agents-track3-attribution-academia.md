...
**Causal Agent Replay / CAR（与 AgentMED 思想最接近的工作）**【事实】
...
220 案例基准，核心论点：失败归因应在**全执行可观测性**下研究。完整轨迹（输入+输出+上下文）比仅输出轨迹归因准确率提升**最高 76%**。直接支撑 AgentMED Quality API 要求完整取证的契约设计。[arXiv:2604.22708](https://arxiv.org/abs/2604.22708)（2026.4）；[GitHub](https://github.com/TraceElephant/TraceElephant)
...
| 对象/思想 | 可作为基线 | 需适配或验证 | 本轮文献样本未见相同组合 |
|---|---|---|---|
| Who&When / TRAIL / TraceElephant 基准 | 作为 AgentMED-for-Agents 归因模块的**验收考题**（已有 ground truth） | | |
| MAST 14 模式 + TRAIL 错误分类法 | 失败模式词汇表，喂给取证与案例库分类 | | |
| AdaMAST 自适应分类法思路 | | 从"固定分类法"改为"内置 MAST 起步+从自身日志长出专属分类法" | |
| "读日志猜"式 LLM judge 归因 | | 只能当**假设生成器**（举证线索），不能当裁决者——学术数据（11–14%）为 AgentMED "裁判≠运动员+实验归因"提供了硬证据 | |
| AgenTracer 反事实重放+故障注入 | | 故障注入可用于**探针集构造**（合成已知根因的失败案例，检验归因模块） | |
| 干预式归因（CAR、A2P、Ma et al.） | 提供反事实/干预思想参考 | 需在真实 Agent 与外部副作用条件下验证 | 本轮文献样本未见“版本快照×冻结探针×Δ+CI”的完全相同配置级实现 |
| Wilson 下界信任账本 | | 阈值、独立性、漂移和晋升语义需单独校准 | 本轮文献样本未见相同治理组合；不能据此视为已验证科学共识 |
| 步级归因（哪个 agent/哪一步错了） | | | **AgentMED 现有三层（prompt/知识库/模型）是配置级归因，agent 场景需要补"轨迹内步级定位"这一粒度** |
...
1. **在真实 workload 中验证实验归因路线。** 本轮论文样本显示，特定基准上的轨迹归因仍不可靠，A2P、CAR、Ma et al. 等引入了反事实或干预。AgentMED 可直接借鉴其方法与开源实现，但配置级双臂实验和轨迹步级定位是不同问题；真实 Agent、外部工具副作用与成本条件下的有效性仍未知，默认先坚持可识别的单因素对照。
...
3. **LLM 归因器降级为"举证助手"，裁决权留给实验。** 学术数据给 AgentMED 既有铁律提供了量化背书；可集成 AgenTracer-8B 这类专用 tracer 生成归因假设（查哪个层、哪个组件），再交给双臂实验证实/证伪——三态裁决的机制不变。
4. **TraceSource 契约必须报告观测完整性。** TraceElephant 在特定 benchmark 中报告全观测最高提升 76%；Agent 场景应尽量收集 OTel span 级输入、输出、工具 I/O 与环境状态。无法收全时必须标记缺失并允许 `UNKNOWN`，不能把该数字当作通用提升保证。
5. **探针集构造可用故障注入。** AgenTracer 用程序化故障注入批量制造"已知根因的失败"，这正是检验 AgentMED 归因模块自身的回归考题来源（类似现有客服场景三案的泛化验证，但可规模化合成）。
6. **用学术基准做归因模块的独立验收。** Who&When/TRAIL 是带 ground truth 的现成考题，AgentMED-for-Agents 若补步级归因能力（轨迹内定位），应在这两个基准上报数，以第三方可复验的方式证明不低于 SOTA。

## 6. 当前综合结论

本轮论文样本表明，单次读轨迹做失败归因在特定 benchmark 上仍不可靠；完整取证、反事实干预和重复运行值得作为产品机制继续验证。AgentMED 还需要分别证明配置级归因、步级定位、失败分类和真实工作负载表现，不能用“先行者”或论文方向替代实际验收。
