...
> **阅读口径更新（2026-08-10）**：本文是限定论文样本的研究快照，不是 CaseLoop 已达到论文结果的证明。论文中的“首次/首个/前沿”应理解为作者或本轮检索口径；不同 benchmark、失败子集和可观测条件下的数字不能直接外推到生产。产品取舍以 `docs/product-principles.md` 为准，配置级因果归因与轨迹步级定位必须分开验收。
...
1. **在真实 workload 中验证实验归因路线。** 本轮论文样本显示，特定基准上的轨迹归因仍不可靠，A2P、CAR、Ma et al. 等引入了反事实或干预。CaseLoop 可直接借鉴其方法与开源实现，但配置级双臂实验和轨迹步级定位是不同问题；真实 Agent、外部工具副作用与成本条件下的有效性仍未知，默认先坚持可识别的单因素对照。
...
本轮论文样本表明，单次读轨迹做失败归因在特定 benchmark 上仍不可靠；完整取证、反事实干预和重复运行值得作为产品机制继续验证。CaseLoop 还需要分别证明配置级归因、步级定位、失败分类和真实工作负载表现，不能用“先行者”或论文方向替代实际验收。
