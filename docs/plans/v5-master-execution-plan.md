...
> R2 只可报告 `contract=PASS`、`replay=PASS`；C0 与 C1 已关闭（C0 semantic series `a14784a` + `903e954`；C1 semantic subject `3dc7339`，post-commit verifier PASS，P0=0/P1=0，确定性再生成与 shadow parity 103 cases PASS）；C2 `ELIGIBLE / NOT STARTED`；C3–C5、D2、R3、R4 与
...
| C0–C5 | C0/C1 `DONE`；C2 `ELIGIBLE / NOT STARTED`；C3–C5 `LOCKED` | 完成模块化单体、单 PostgreSQL UoW 与 compatibility façade 收敛前禁止领域扩张 |
...
| C2 records/event specs/graph verifier foundation | `ELIGIBLE / NOT STARTED` | 前一 wave 独立 Exit；详见 convergence plan |
| C3–C5 convergence | `LOCKED` | 前一 wave 独立 Exit；详见 convergence plan |
...
1. C0 已关闭（`a14784a`+`903e954`）、C1 已关闭（`3dc7339`；compiler + 2020-12 schemas + shadow parity PASS）。先完成 C1 gate review，再开始 C2 foundation extraction（records/event specs/graph verifier）。
