...
> R2 只可报告 `contract=PASS`、`replay=PASS`；C0–C3 已关闭（C0 `a14784a`+`903e954`；C1 `3dc7339`；C2 `d2c3f18`；C3 `3adaac0`，import-cycle 消除 + capability 接线 C1 输出，post-commit verifier PASS，P0=0/P1=0）；C4 `ELIGIBLE / NOT STARTED`；C5、D2、R3、R4 与
...
| C0–C5 | C0–C3 `DONE`；C4 `ELIGIBLE / NOT STARTED`；C5 `LOCKED` | 完成模块化单体、单 PostgreSQL UoW 与 compatibility façade 收敛前禁止领域扩张 |
...
| C4 convergence | `ELIGIBLE / NOT STARTED` | 前一 wave 独立 Exit；详见 convergence plan |
| C5 convergence | `LOCKED` | 前一 wave 独立 Exit；详见 convergence plan |
...
1. C0（`a14784a`+`903e954`）、C1（`3dc7339`）、C2（`d2c3f18`）、C3（`3adaac0`；import-cycle 消除 + capability 接线）均已关闭。先完成 C3 gate review，再开始 C4 generated transport cutover。
