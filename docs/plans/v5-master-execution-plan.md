...
> R2 只可报告 `contract=PASS`、`replay=PASS`；C0、C1 与 C2 已关闭（C0 `a14784a`+`903e954`；C1 `3dc7339`；C2 `d2c3f18`，foundation extraction，post-commit verifier PASS，P0=0/P1=0）；C3 `ELIGIBLE / NOT STARTED`；C4–C5、D2、R3、R4 与
...
| C0–C5 | C0/C1/C2 `DONE`；C3 `ELIGIBLE / NOT STARTED`；C4–C5 `LOCKED` | 完成模块化单体、单 PostgreSQL UoW 与 compatibility façade 收敛前禁止领域扩张 |
...
| C3 convergence | `ELIGIBLE / NOT STARTED` | 前一 wave 独立 Exit；详见 convergence plan |
| C4–C5 convergence | `LOCKED` | 前一 wave 独立 Exit；详见 convergence plan |
...
1. C0（`a14784a`+`903e954`）、C1（`3dc7339`）、C2（`d2c3f18`；foundation records/events/bindings/receipts/graph）均已关闭。先完成 C2 gate review，再开始 C3 capability/import-cycle elimination 与 coordinator/service 分解。
