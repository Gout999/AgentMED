...
- **工具面**：mcp-case-admin（case.list/get/timeline/claim/submit_suggestion）、mcp-notification（升级人工时发消息）。
...
- **工具面**：mcp-case-admin（app.logs/app.feedback 取证，已 PII 脱敏）。
...
- **工具面**：mcp-eval-runner（gate.run/gate.report）、mcp-release-admin（读面）、trust-ledger（查账本状态）。
...
- **工具面**：mcp-casebase-knowledge（kb.upsert/badcase_search/holdout_get，唯一写权限持有者）、mcp-notification（周报）。
...
- **工具面**：mcp-eval-runner（experiment.plan/查询报告）、mcp-case-admin（读面）。
